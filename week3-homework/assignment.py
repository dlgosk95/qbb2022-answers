...
ax[0].hist(read_depth, bins = np.arange(0, max(read_depth) + 1, 1))
ax[1].hist(genotype_quality, bins = np.arange(0, max(genotype_quality) + 1, 1))
ax[2].hist(allele_frequency)
ax[3].bar(x, height) 
plt.xticks(rotation=30, ha='right') # rotate the lables
